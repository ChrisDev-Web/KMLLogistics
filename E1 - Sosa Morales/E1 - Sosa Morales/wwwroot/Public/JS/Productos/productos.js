(function () {
    ...
    function renderRows(tbody, items, mode) {
        ...
        tbody.innerHTML = arr.map(function (r) {
            ...
            var cells = '<td>' + r.idProduct + '</td><td>' + escapeHtml(r.name) + '</td><td>' + escapeHtml(r.categoryName) + '</td><td>' + escapeHtml(r.brandName) + '</td><td>S/ ' + Number(r.cost).toFixed(2) + '</td><td><strong>S/ ' + Number(r.salePrice).toFixed(2) + '</strong></td>';
            if(mode === 'inactive') cells = '<td>' + r.idProduct + '</td><td>' + escapeHtml(r.name) + '</td><td>S/ ' + Number(r.salePrice).toFixed(2) + '</td>';
    ...
    function loadList(urlObj, tbodyId, searchVal, pageVal, pageSizeVal, mode) {
        fetchJson(buildQuery(urlObj, { search: searchVal, page: pageVal, pageSize: pageSizeVal })).then(function (d) {
    ...
    function resetForm() { var f = qs('proForm'); if (f) f.reset(); qs('proFormId').value = '0'; }

    function saveForm() {
        var f = qs('proForm'); if (!f.checkValidity()) { f.reportValidity(); return; }
        
        var data = {
            IdProduct: qs('proFormId').value,
            Name: qs('proName').value.trim(),
            IdCategory: qs('proCategory').value,
            IdBrand: qs('proBrand').value,
            Cost: qs('proCost').value,
            ProfitPercentage: qs('proProfit').value,
            Weight: qs('proWeight').value, Height: qs('proHeight').value, Width: qs('proWidth').value, Length: qs('proLength').value,
            Description: qs('proDescription').value.trim()
        };

        postAction(urls().save, data).then(function (r) {
    ...
    function openDetailModal(id) {
        fetchJson(buildQuery(urls().get, { id: id })).then(function (r) {
            ...
            qs('proDetailBody').innerHTML = rows.map(function (row) {
